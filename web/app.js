...
function settingsNotice() {
  ...
  if (!config.providerReady) return `<div class="notice">${icon('info')}<p>Connect the video model in <button class="quiet small" data-view="settings">Engine settings</button> to generate new footage. You can import your own clips and export now.</p></div>`;
  return '';
}
function studio() {
...
async function generate() {
  ...
  modal('Render this film?', `<p><strong>${escape(project.title)}</strong><br>${project.scenes.length} scenes · ${totalSeconds()} seconds · ${escape(project.resolution)}</p><p>${generatedScenes ? `${generatedScenes} scenes will be sent to fal for paid video generation. Your prompts and any reference images are shared with the provider. Charges depend on your provider account and selected output.` : 'This render uses your imported clips. No paid AI generation request is needed.'}</p>`, generatedScenes ? 'Use credits & generate' : 'Assemble video', async () => {
